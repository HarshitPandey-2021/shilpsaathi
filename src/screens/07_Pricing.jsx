...
export default function PricingScreen() {
  ...
  const initialCost = Number(productData.raw_material_cost ?? productData.estimated_material_cost ?? 220);
  const initialHours = Number(productData.hours_spent ?? productData.estimated_labor_hours ?? 5);
  ...
  useEffect(() => {
    ...
    const calculate = async () => {
      ...
      try {
        ...
        const suggested = Math.max(30, Math.round((prodCost + margin) / 10) * 10);
        const minP = Math.max(20, Math.round((suggested * 0.9) / 10) * 10);
        const maxP = Math.max(minP + 20, Math.round((suggested * 1.25) / 10) * 10);
  ...
  return (
    <div className="space-y-4 animate-fade-in-up">
      ...
      <div className="bg-amber-100/80 border border-mustard/60 p-4 rounded-3xl text-center space-y-2 shadow-sm">
        ...
        <div className="flex items-center justify-center gap-2">
          <h3 className="text-3xl font-black text-charcoal">
            ₹{breakdown?.price_min ?? productData.price_min ?? 0} – ₹{breakdown?.price_max ?? productData.price_max ?? 0}
          </h3>
        </div>
        <div className="flex items-center justify-center gap-2 pt-1">
          ...
          <span className="text-sm font-black text-terracotta bg-white px-2.5 py-0.5 rounded-full border border-terracotta/30 shadow-xs">
            ₹{breakdown?.suggested_price ?? productData.final_price ?? 0}
          </span>
          <button
            onClick={applySuggested}
            className="text-[10px] font-bold text-forest bg-green-100 hover:bg-green-200 border border-green-300 px-2 py-0.5 rounded-md transition cursor-pointer"
          >
            लागू करें (Apply)
          </button>
        </div>
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            <div className="flex items-center gap-1">
              ...
              <input
                ...
                max="10000"
                value={rawCost}
                onChange={e => setRawCost(Math.max(0, Number(e.target.value) || 0))}
                className="w-16 px-1.5 py-0.5 text-right font-black text-terracotta border border-stone-200 rounded-md bg-stone-50 focus:bg-white outline-none focus:border-terracotta"
              />
            </div>
          </div>
          <input
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                className="w-16 px-1.5 py-0.5 text-right font-black text-terracotta border border-stone-200 rounded-md bg-stone-50 focus:bg-white outline-none focus:border-terracotta"
              />
              <span className="font-semibold text-stone-500">hrs</span>
            </div>
          </div>
          <input
      ...
      <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm">
        ...
        <div className="flex items-center gap-1 border-b-2 border-terracotta pb-1">
          ...
          <input 
            ...
            placeholder={String(breakdown?.suggested_price || 890)}
          />
        </div>
      </div>
      ...
      <button 
        onClick={nextStep} 
        className="w-full py-4 bg-terracotta hover:bg-[#8e3e29] text-white rounded-2xl font-bold flex justify-center items-center gap-2 shadow-lg shadow-terracotta/25 active:scale-[0.98] transition cursor-pointer"
      >
