...
export default function FinalListingScreen() {
  const { productData, goToStep, t } = useCraft();
    ...
    <div className="space-y-4 text-center animate-fade-in-up">
      ...
      <div className="bg-white p-4 rounded-3xl border border-stone-200 text-left shadow-lg space-y-2">
        ...
        <div className="flex items-baseline justify-between pt-1">
          ...
          <span className="text-[10px] text-forest font-bold bg-green-50 px-2 py-0.5 rounded-md border border-green-200">Ready</span>
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-2">
        <button
          onClick={handleShare}
          className="flex-1 py-4 bg-terracotta text-white rounded-2xl font-bold flex items-center justify-center gap-2 text-xs shadow-md hover:bg-[#8e3e29] active:scale-[0.98] transition"
        >
        ...
        <button
          onClick={() => goToStep(2)}
          className="px-5 py-4 bg-stone-200 hover:bg-stone-300 rounded-2xl font-bold text-xs active:scale-[0.98] transition"
        >
          {t.homeBtn}
        </button>
      </div>
